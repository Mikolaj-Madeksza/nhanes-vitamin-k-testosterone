...
analysis_clean <- analysis_clean %>%
  mutate(
    vk_density_half_sd = (vk_density - mean(vk_density, na.rm = TRUE)) / half_sd
  )

design_1116 <- update(
  design_1116,
  vk_density_half_sd = analysis_clean$vk_density_half_sd
)

m1 <- svyglm(
  lbxtst ~ vk_density_half_sd + ridageyr,
  design = design_1116
)

m1_tab <- tidy(m1, conf.int = TRUE) %>%
  filter(term == "vk_density_half_sd") %>%
  transmute(
    Model = "Model 1 (Age-adjusted)",
    Exposure = "Vitamin K density (per 1/2 SD)",
    Beta = estimate,
    CI_low = conf.low,
    CI_high = conf.high,
    P_value = p.value
...
log_vk_half_sd_value <- log_vk_sd_raw / 2

analysis_clean <- analysis_clean %>%
  mutate(
    log_vk_half_sd = (log_vk - mean(log_vk, na.rm = TRUE)) /
      log_vk_half_sd_value
  )

design_1116 <- update(
  design_1116,
  log_vk_half_sd = analysis_clean$log_vk_half_sd
)

m1_logvk <- svyglm(
  lbxtst ~ log_vk_half_sd + ridageyr,
  design = design_1116
)

m2_logvk <- svyglm(
  lbxtst ~ log_vk_half_sd +
    ridageyr +
    race +
    indfmpir +
    smoking +
...
extract_log <- function(model, model_name) {
  tidy(model, conf.int = TRUE) %>%
    filter(term == "log_vk_half_sd") %>%
    transmute(
      Model = model_name,
      Exposure = "Log vitamin K density (per 1/2 SD)",
      Beta = estimate,
      CI_low = conf.low,
      CI_high = conf.high,
      P_value = p.value
...
m1_lowT <- svyglm(
  lowt ~ vk_density_half_sd + ridageyr,
  design = design_1116,
  family = quasibinomial()
)

m2_lowT <- svyglm(
  lowt ~ vk_density_half_sd +
    ridageyr +
    race +
    indfmpir +
    smoking +
...
design_energy <- update(design_energy,
                        vk_density_half_sd = analysis_energy$vk_density_half_sd)
...
analysis_trim_vk <- analysis_trim_vk %>%
  mutate(
    vk_density_half_sd_trim =
      (vk_density - vk_mean_trim) / (vk_sd_trim / 2)
  )

design_trim_vk <- update(
  design_trim_vk,
  vk_density_half_sd_trim = analysis_trim_vk$vk_density_half_sd_trim
)

m2_trim_vk <- svyglm(
  lbxtst ~ vk_density_half_sd_trim +
    ridageyr +
    race +
    indfmpir +
    smoking +
...
tab_m2_trim_vk <- tidy(m2_trim_vk, conf.int = TRUE) %>%
  filter(term == "vk_density_half_sd_trim")
...
analysis_trim_t <- analysis_trim_t %>%
  mutate(
    vk_density_half_sd_trim_t =
      (vk_density - vk_mean_trim_t) / (vk_sd_trim_t / 2)
  )

design_trim_t <- update(
  design_trim_t,
  vk_density_half_sd_trim_t =
    analysis_trim_t$vk_density_half_sd_trim_t
)

m2_trim_t <- svyglm(
  lbxtst ~ vk_density_half_sd_trim_t +
    ridageyr +
    race +
    indfmpir +
    smoking +
...
tab_m2_trim_t <- tidy(m2_trim_t, conf.int = TRUE) %>%
  filter(term == "vk_density_half_sd_trim_t")
...
m2_interaction_obesity <- svyglm(
  lbxtst ~ vk_density_half_sd * obesity +
    ridageyr +
    race +
    indfmpir +
    smoking +
...
interaction_p_obesity <- tidy(m2_interaction_obesity) %>%
  filter(grepl("vk_density_half_sd:obesity", term)) %>%
  pull(p.value)
...
m2_interaction_age <- svyglm(
  lbxtst ~ vk_density_half_sd * age_group +
    ridageyr +
    race +
    indfmpir +
    smoking +
...
interaction_p_age <- tidy(m2_interaction_age) %>%
  filter(grepl("vk_density_half_sd:age_group", term)) %>%
  pull(p.value)
...
m2_interaction_ir <- svyglm(
  lbxtst ~ vk_density_half_sd * ir_group +
    ridageyr +
    race +
    indfmpir +
    smoking +
...
interaction_p_ir <- tidy(m2_interaction_ir) %>%
  filter(grepl("vk_density_half_sd:ir_group", term)) %>%
  pull(p.value)
...
m2_lowT_interaction_age <- svyglm(
  lowt ~ vk_density_half_sd * age_group +
    ridageyr +
    race +
    indfmpir +
    smoking +
...
m2_trim_interaction_age <- svyglm(
  lbxtst ~ vk_density_half_sd_trim * age_group +
    ridageyr +
    race +
    indfmpir +
    smoking +
...
interaction_p_trim_age <- tidy(m2_trim_interaction_age) %>%
  filter(grepl("vk_density_half_sd_trim:age_group", term)) %>%
  pull(p.value)
